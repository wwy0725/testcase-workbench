...
本 agent 依赖外部 skill/CLI 访问公司平台。这些 skill **不是本项目的资产**，是新平台需按 SKILL.md「前置依赖清单」安装的。调用前**必须探测**，探测不到就降级，不中断流程：

- **Windows**：`Test-Path "<skill脚本路径>"` 或 `Get-Command <cli名> -ErrorAction SilentlyContinue`
- **macOS/Linux**：`command -v <cli名>` 或 `test -f "<skill脚本路径>"`
- skill 脚本路径不写死 `~/.claude/skills/`（用户目录/平台不同会炸），用 `<skill_dir>` 占位符，按实际安装位置解析

**降级规则**（探测失败时）：
- 探测不到对应 skill → 提示用户"该平台 skill 未安装"，请用户把文档内容直接粘贴进来
- 粘贴内容 → 照常进入阶段 3 建立工作区，**不因缺 skill 中断**
- 认证失败 → 自动重试一次 `oauth` 命令（17work/Walle 类），不向用户解释细节

**庄周**（产品稿/设计稿，npx CLI，探测 `npx` 可用即可）：
```bash
npx -y --registry=http://npm.dc.servyou-it.com @servyou-ai/chuangtzu-cli@latest -u "<url>" -t md
```

**禅道**（需求正文，zentao-skill 全局 skill）：
```bash
node <skill_dir>/zentao-skill/scripts/cli.cjs zentao url "<url>"
# 探测：command -v zentao-skill 或 test -f "<skill_dir>/zentao-skill/scripts/cli.cjs"
```

**Walle**（接口文档，walle-skill 全局 skill）：
```bash
node <skill_dir>/walle-skill/scripts/cli.cjs walle url "<url>"
# 探测：test -f "<skill_dir>/walle-skill/scripts/cli.cjs"
```

**Baymax**（历史测试沉淀，**仅在用户提供 URL 时才查**，usecase-skill 全局 skill）：
```bash
node <skill_dir>/usecase-skill/scripts/cli.cjs usecase url "<url>"
# 探测：test -f "<skill_dir>/usecase-skill/scripts/cli.cjs"
```

**17work Spec**（资料包批量下载，npx CLI）：
```bash
...
> 各 skill 具体命令语法，见本 skill 的 `references/` 或对应 skill 的 SKILL.md。`<skill_dir>` 是占位符，解析为实际安装目录（通常是 `~/.claude/skills/` 或环境指定的 skill 目录）。
